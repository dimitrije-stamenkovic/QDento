...
bool DbNotification::insert(const Notification& n){
    ...
    db.newStatement(
        "INSERT INTO notification (date, patient_rowid, dentist_rowid, description) VALUES (?,?,?,?)"
    );

    db.bind(1, n.date.to8601());
    db.bind(2, n.patientRowid);
    db.bind(3, User::dentist().rowID);
    db.bind(4, n.description);

    return db.execute();
}

std::vector<Notification> DbNotification::get(long long dentist_rowid)
{
    Db db;
    ...
    db.newStatement(
        ...
        "WHERE dentist_rowid=? ORDER BY notification.date ASC"
    );
