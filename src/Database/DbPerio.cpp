...
PerioStatus DbPerio::getPerioStatus(long long patientRowId, Date date)
{
    ...
    std::string query = "SELECT rowid, dentist_rowid, date, data FROM periostatus WHERE"
    ...
    for(Db db(query); db.hasRows();)
    {
        ...
        perioStatus.dentist_rowid = db.asRowId(1);
...
PerioStatus DbPerio::getPerioStatus(long long rowid)
{
    ...
    Db db(
        "SELECT rowid, dentist_rowid, date, data FROM periostatus WHERE"
...
{
    std::string query =
        "INSERT INTO periostatus (dentist_rowid, patient_rowid, date, data) VALUES (?,?,?,?)"
        ;
    ...
    db.bind(3, perioStatus.date.to8601());
    db.bind(4, Parser::write(perioStatus));
