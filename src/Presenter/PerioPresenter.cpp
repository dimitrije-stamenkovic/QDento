    ...
    m_toothStatus(DbPerio::getStatus(patient->rowid, m_perioStatus.date))
{
    ...
    if (m_perioStatus.date != Date::currentDate() && !rowId) //if its not todays measurment and it is new document
    {
        ...
        (QObject::tr("A previous periodontal measurment has been found. "
            "Do you want to load the previous results?").toStdString());
...
void PerioPresenter::teethViewChanged(bool showLowerTeeth)
{
    this->showLowerTeeth = showLowerTeeth;
}
...
void PerioPresenter::setDataToView()
{
    ...
    view->setTeethView(showLowerTeeth);
...
TabName PerioPresenter::getTabName()
{
    return isNew() ? 
    TabName{ 
        .header = QObject::tr("New Periodontal Measurment").toStdString(), 
        .footer = patient->firstLastName(), 
        .indicatorColor = patient->colorNameRgb 
    }
    :
    TabName{
        .header = QObject::tr("Periodontal Measurment ").toStdString() + m_perioStatus.date.toLocalFormat(),
        .footer = patient->firstLastName() ,
        .indicatorColor = patient->colorNameRgb
    };
}
