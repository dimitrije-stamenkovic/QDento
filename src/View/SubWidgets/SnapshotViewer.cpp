...
void SnapshotViewer::setSnapshots(const std::vector<Snapshot>& snapshots)
{
	m_snapshots = snapshots;

    if (snapshots.empty()) {

        displaySnapshotToView(Snapshot());
...
const Snapshot* SnapshotViewer::getCurrentSnapshot() const
{
...
void SnapshotViewer::displaySnapshotToView(const Snapshot& s)
{
    for (int i = 0; i < 32; i++)
    {
    ...
    ui.dateLabel->setText(
        s.date.isDefault() ? "" :
        QString("<b>" + tr("Date") + ":</b> ") + s.date.toLocalFormat().c_str());

    ui.procedureLabel->setText(
        s.procedure_name.empty() ? "" : 
        QString("<b>" + tr("Procedure") + ":</b> ") + s.procedure_name.c_str()
    );

    ui.diagnosisLabel->setText(
        s.procedure_diagnosis.empty() ? "" :
        QString("<b>" + tr("Diagnosis") + ":</b> ") + s.procedure_diagnosis.c_str()
    );
    ...
    if (s.procedure_note.size()) {
        ui.notesLabel->setText(QString("<b>" + tr("Notes") + ":</b> ") + s.procedure_note.c_str());
    }
    else {
        ui.notesLabel->clear();
    }
