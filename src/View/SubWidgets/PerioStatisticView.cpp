...
void PerioStatisticView::setPerioStatistic(const PerioStatistic& stat, const PerioStatistic* const prev)
{
	...
	QString diagnosis = tr(perioStage[static_cast<int>(stat.stage)].data());

	if (stat.stage != Stage::Healthy)
	{
		diagnosis += stat.localized ? tr(" localized") : tr(" generalized");
		diagnosis += tr(" periodontitis");
	}

	ui.diagnosis->setText(tr("Diagnosis: ") + diagnosis);

	QString risk = tr("Risk Assessment: ");
	risk += tr(perioRisk[static_cast<int>(stat.risk)].data());
	risk += tr(" risk");
